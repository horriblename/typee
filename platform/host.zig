...
const Context = extern struct { ptr: *c.gcc_jit_context };
const Type = extern struct { ptr: *c.gcc_jit_type };
const Function = extern struct { ptr: *c.gcc_jit_function };
const Param = extern struct { ptr: *c.gcc_jit_param };

fn tempNullException() noreturn {
    @panic("null exception");
}

pub export fn roc_fx_acquireContext() Context {
    return Context{ .ptr = c.gcc_jit_context_acquire() orelse tempNullException() };
}

pub export fn roc_fx_getVoidType(ctx: Context) Type {
    return Type{ .ptr = c.gcc_jit_context_get_type(ctx.ptr, c.GCC_JIT_TYPE_VOID) orelse tempNullException() };
}

pub export fn roc_fx_getIntType(ctx: Context) Type {
    return Type{ .ptr = c.gcc_jit_context_get_type(ctx.ptr, c.GCC_JIT_TYPE_INT) orelse tempNullException() };
}

const default_allocator = std.heap.page_allocator;

fn c_str(string: RocStr) [:0]u8 {
    return default_allocator.dupeZ(u8, string.asSlice()) catch @panic("allocation failure");
}

const NewFunctionArgs = extern struct {
    context: Context,
    returnType: Type,
    exportFlag: bool,
    name: RocStr,
