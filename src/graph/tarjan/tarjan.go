// from https://github.com/looplab/tarjan
//
// Modified to use generics instead of interface{}
//
...
func Connections[Node comparable](graph map[Node][]Node) [][]Node {
	g := &data[Node]{
		graph: graph,
		nodes: make([]node, 0, len(graph)),
		index: make(map[Node]int, len(graph)),
	}
	for v := range g.graph {
...
type data[Node comparable] struct {
	graph  map[Node][]Node
	nodes  []node
	stack  []Node
	index  map[Node]int
...
func (data *data[Node]) strongConnect(v Node) *node {
