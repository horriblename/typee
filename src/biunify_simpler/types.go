...
func (self *Variable) newUpperBound(ub ConcreteType) fun.Result[SimpleType] {
	self.occursCheck(ub, true)
...
func (self *Variable) newLowerBound(lb ConcreteType) error {
	...
	rep.lowerBound = lubConcrete(rep.lowerBound, lb)
...
func glbConcrete(lhs ConcreteType, rhs ConcreteType) ConcreteType {
	type C = ConcreteType
	if _, rhs, ok := matchPair[Top, C](lhs, rhs); ok {
		...
		return Func{fun.ZipMap(lhs.Args, rhs.Args, lub), glb(lhs.Ret, rhs.Ret)}
	} else if lhs, rhs, ok := matchPair[Record, Record](lhs, rhs); ok {
		lhsMap := namedTypesToMap(lhs.Fields)
		rhsMap := namedTypesToMap(rhs.Fields)
		mergedMap := mergeMapWith(lhsMap, rhsMap, glb)
		return Record{mapToNamedTypes(mergedMap)}
...
func lubConcrete(lhs ConcreteType, rhs ConcreteType) ConcreteType {
	type C = ConcreteType
	if lhs, rhs, ok := matchPair[Bot, C](lhs, rhs); ok {
		...
		return Func{fun.ZipMap(lhs.Args, rhs.Args, glb), lub(lhs.Ret, rhs.Ret)}
	} else if lhs, rhs, ok := matchPair[Record, Record](lhs, rhs); ok {
		...
		for _, lhsField := range lhs.Fields {
			if rhsField, ok := rhsMap[lhsField.Name]; ok {
				...
				merged = append(merged, NamedType{
					Name: lhsField.Name,
					Type: lub(lhsField.Type, rhsField),
				})
			}
		}
...
func glb(lhs SimpleType, rhs SimpleType) SimpleType {
	if lhs, rhs, ok := matchPair[ConcreteType, ConcreteType](lhs, rhs); ok {
		...
		rhs.newUpperBound(lhs)
	}
}
func lub(SimpleType, SimpleType) SimpleType {}
